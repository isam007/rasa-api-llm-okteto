        ...
        if (Object.hasOwnProperty.call(response[i], "text")) {
          if (response[i].text != null) {
            ...
            if (
              ...
              // }
              if (!botResponse) {
                let textWithLinks = response[i].text.replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank">$1</a>');
                botResponse = `<img class="botAvatar" src="./static/img/botAvatar_old.png"/><p class="botMsg">${textWithLinks}</p><div class="clearfix"></div>`;
              }
            }
...
function actionTrigger() {
  $.ajax({
    ...
    url: `https://rasa-ibot-isam007.cloud.okteto.net/conversations/${sender_id}/execute`,
    type: "POST",
    contentType: "application/json",
    data: JSON.stringify({
      name: action_name,
...
function customActionTrigger() {
  $.ajax({
    ...
    url: "https://action-server-ibot-isam007.cloud.okteto.net/webhook/",
    type: "POST",
    contentType: "application/json",
    data: JSON.stringify({
